...
/*     std::cout << "range: " << range << std::endl;
    ...
    std::cout << "Before Decoding: low = " << low << ", high = " << high << ", code = " << code << std::endl; */

    // Calcular o valor acumulado (scaled code) dentro do intervalo atual
    uint64_t scaledValue = (((code - low)*1.0) / range*1.0) * totalFreq;
/*     std::cout << "code: " << code << ", low: " << low << ", totalFreq: " << totalFreq << std::endl;
    std::cout << "scaledValue: " << scaledValue << std::endl;
    std::cout << (((code - low)*1.0) / range*1.0) << std::endl;
 */
...
/*     std::cout << "After Decoding: low = " << low << ", high = " << high << ", code = " << code << std::endl;
    std::cout << "symbolLow: " << symbolLow << ", symbolHigh: " << symbolHigh << std::endl; */

    // Normalizar o intervalo
    normalize();
/* 
    std::cout << "------------------------ Dodificado:" << symbol << std::endl;
 */
    return symbol;
}
